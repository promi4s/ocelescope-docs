...
:::caution[Progress is lost]
After a crash, all progress is lost: the OCELs you uploaded and any resources you computed in the session are gone, and you have to start over.
:::
...
:::note
On macOS and Windows, Docker runs inside a virtual machine, so a container can only use as much memory as that VM is given. If the command above does not help, raise the VM's memory through Docker Desktop as described below.
:::

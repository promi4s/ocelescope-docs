...
![The output of the OCEL Graph](../../../assets/ocel-graph.png)
...
Our plugin should return an **OCEL Graph**. In Ocelescope, custom outputs are implemented as [resources](/plugin-development/resources/).
...
Download [`util.py`](/downloads/ocelGraphTutorial/util.py) and add it next to `plugin.py`:

...
A `Graph` is laid out through its `layout_config`. Instead of hand-tuning layout
options, prefer one of the ready-made presets exported from
`ocelescope.visualization.default.graph`:

| Preset | Best for |
| --- | --- |
| `DIRECTED_ELK_GRAPH_LAYOUT` | Large directed model graphs (Petri nets, DFGs) — high-quality ELK layered layout. |
| `DIRECTED_GRAPH_LAYOUT` | Directed model graphs where edge direction shows progression (Graphviz `dot`). |
| `ORDERED_TREE_LAYOUT` | Rooted trees where sibling order matters (process trees, parse trees). |
| `RADIAL_GRAPH_LAYOUT` | Object-to-object / relationship graphs with no single process direction. |
| `FORCE_GRAPH_LAYOUT` | Dense or cyclic graphs where layering doesn't produce a readable result. |

You can still build a custom layout by passing a `GraphvizLayoutConfig` or
`ElkLayoutConfig` to `layout_config` when you need full control.
